...
import { X, Trophy, Users, Eye, Calendar, Clock, Instagram, ExternalLink, ChevronDown } from 'lucide-react';
...
export function CompetitionDetail({ competition, onClose, onSimulationSet, simulationData, teams = [], onTeamClick }: CompetitionDetailProps) {
      ...
      <DrawerContent className="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 border-t border-slate-700/50 max-h-[90vh] rounded-t-3xl">
        ...
        <div className="overflow-y-auto max-h-[85vh] scrollbar-hide">
            ...
            <div className="flex flex-col items-center text-center space-y-4">
              ...
              <div className="relative">
                ...
                {competition.logo ? (
                  ...
                  <div className="relative w-20 h-20 bg-gradient-to-br from-purple-500 to-blue-600 rounded-2xl flex items-center justify-center shadow-2xl border-2 border-white/20">
              ...
              <div>
                ...
                {competition.city && (
                  <p className="text-white/90 text-base font-medium mb-2">{competition.city}</p>
                )}
                ...
                <div className="flex flex-wrap justify-center gap-1.5 mb-3">
                  ...
                  <div className="flex items-center gap-1 bg-blue-500/20 backdrop-blur-sm px-2 py-1 rounded-full text-xs border border-blue-400/30">
                    <Users className="h-3 w-3 text-blue-300" />
                    <span className="text-blue-200">{competition.lineup.length} Teams</span>
                  </div>
                  
                  <div className={`flex items-center gap-1 backdrop-blur-sm px-2 py-1 rounded-full text-xs border font-medium ${
          ...
          <div className="px-4 pb-4">
            <div className="grid grid-cols-3 gap-2">
              ...
              <div className="bg-gradient-to-br from-blue-500/15 to-blue-600/10 border border-blue-400/30 rounded-xl p-3 text-center">
                ...
                <div className="text-xl font-black text-white mb-0.5">{competition.judges.length}</div>
              ...
              <div className={`rounded-xl p-3 text-center border ${
                ...
                <div className={`rounded-full p-2 w-fit mx-auto mb-2 ${
                ...
                <div className={`text-xl font-black mb-0.5 ${competition.bid_status ? 'text-green-300' : 'text-slate-300'}`}>
                  {competition.bid_status ? 'YES' : 'NO'}
                </div>
                <div className={`text-xs font-medium ${competition.bid_status ? 'text-green-300' : 'text-slate-300'}`}>
                  Bid Event
                </div>
              </div>
            </div>
              ...
              <div className="grid gap-1.5">
                {competition.lineup.map((team, index) => (
                  <div 
                    ...
                    className="flex items-center gap-2 bg-slate-700/30 rounded-lg px-2.5 py-2 text-slate-300 text-sm cursor-pointer hover:bg-slate-600/50 transition-colors active:scale-[0.98]"
                  >
          ...
          <div className="px-4 pb-4">
            ...
            {isFutureCompetition ? (
              <div className="bg-gradient-to-br from-slate-800/50 to-slate-700/30 border border-slate-600/40 rounded-xl p-3 space-y-3">
                ...
                {canSaveSimulation && (
                  <button
                    ...
                    className={`w-full px-4 py-3 rounded-lg transition-colors font-semibold min-h-[44px] ${
                      showSuccessMessage 
              ...
              <div className="space-y-2">
                {firstPlaceTeam && (
                  <div 
                    ...
                    className="flex items-center gap-2 bg-gradient-to-r from-yellow-600/20 to-yellow-400/10 border border-yellow-600/30 rounded-xl p-3 cursor-pointer hover:from-yellow-600/30 hover:to-yellow-400/20 transition-all duration-200 active:scale-[0.98]"
                  >
                ...
                {secondPlaceTeam && (
                  <div 
                    ...
                    className="flex items-center gap-2 bg-gradient-to-r from-slate-500/20 to-slate-400/10 border border-slate-500/30 rounded-xl p-3 cursor-pointer hover:from-slate-500/30 hover:to-slate-400/20 transition-all duration-200 active:scale-[0.98]"
                  >
                ...
                {thirdPlaceTeam && (
                  <div 
                    ...
                    className="flex items-center gap-2 bg-gradient-to-r from-orange-600/20 to-orange-400/10 border border-orange-600/30 rounded-xl p-3 cursor-pointer hover:from-orange-600/30 hover:to-orange-400/20 transition-all duration-200 active:scale-[0.98]"
                  >
                ...
                {!firstPlaceTeam && !secondPlaceTeam && !thirdPlaceTeam && (
                  <div className="text-slate-400 text-sm text-center py-6 bg-slate-800/30 rounded-xl border border-slate-600/30">
            ...
            <div className="bg-gradient-to-br from-slate-800/50 to-slate-700/30 border border-slate-600/40 rounded-xl p-3">
              <div className="grid gap-2">
                {competition.judges
                  ...
                  .map((judge, index) => (
                  <div key={index} className="flex items-center gap-2 bg-slate-700/30 rounded-lg px-2.5 py-2">
          ...
          {competition.instagramlink && (
            <div className="px-4 pb-6">
              <a
                ...
                className="group flex items-center justify-center gap-2 bg-gradient-to-r from-pink-500/15 to-purple-500/10 border border-pink-400/30 rounded-xl p-3 hover:from-pink-500/20 hover:to-purple-500/15 transition-all duration-300 active:scale-[0.98]"
              >
