...
const DrawerContent = React.forwardRef<React.ElementRef<typeof DrawerPrimitive.Content>, React.ComponentPropsWithoutRef<typeof DrawerPrimitive.Content>>(({
  ...
  ...props
    ...
    <DrawerOverlay />
    <DrawerPrimitive.Content ref={ref} className={cn("fixed inset-x-0 bottom-0 z-50 flex h-auto flex-col rounded-t-[20px] border bg-background", className)} style={{ maxHeight: 'calc(100dvh - env(safe-area-inset-top, 0px) - 2rem)' }} {...props}>
      <div className="sticky top-0 z-10 flex justify-center bg-inherit rounded-t-[20px] pt-5 pb-4" style={{ touchAction: 'none' }}>
